...
const TutoriaisFalarComGerente: React.FC<Props> = ({ onNavigate, onOpenSupport }) => {
  ...
  const steps = [
    {
      title: "Aceda ao Chat",
      description: "Toque no ícone de suporte oficial no canto inferior para iniciar o contato.",
      icon: "chat"
    },
    {
      title: "Escolha o Assunto",
      description: "Selecione o tópico 'Gerente de Conta' para um atendimento personalizado sobre o seu saldo.",
      icon: "list_alt"
    },
    {
      title: "Conexão Segura",
      description: "Aguarde alguns minutos. Conectamos você ao seu gerente de conta dedicado na BP Angola.",
      icon: "verified_user"
    }
  ];
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-white font-sans text-black flex flex-col">
      <header className="sticky top-0 z-50 bg-white p-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <h1 className="text-base font-bold text-center flex-1 tracking-tight">Atendimento</h1>
      ...
      >
        <div className="max-w-md mx-auto space-y-8">
          <div className="flex items-center gap-4 bg-gray-50 p-5 rounded-[32px] border border-gray-100">
            ...
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Suporte VIP</span>
          ...
          <div className="bg-blue-50 p-6 rounded-[32px] border border-blue-100 flex gap-4">
            <span className="material-symbols-outlined text-blue-500 text-[20px] shrink-0">help</span>
            <div>
              <p className="text-[14px] font-black text-blue-900 mb-1">Horário de Atendimento</p>
              <p className="text-[12px] text-blue-600 font-medium leading-relaxed italic">
                O suporte oficial por gerente está disponível de segunda a sábado, das 09:00 às 18:00 (Luanda).
              </p>
            </div>
          </div>
        </div>
