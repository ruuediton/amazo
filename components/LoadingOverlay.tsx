...
const LoadingOverlay: React.FC<LoadingOverlayProps> = ({ status, message }) => {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-[2px] animate-in fade-in duration-300">
            {status === 'loading' ? (
                <div className="flex flex-col items-center justify-center animate-in zoom-in-95 duration-300">
                    <SpokeSpinner size="w-12 h-12" className="text-black" />
                </div>
            ) : (
                <div className="bg-white/90 backdrop-blur-md rounded-3xl p-6 flex flex-col items-center justify-center min-w-[200px] max-w-[80vw] border border-white/20 shadow-2xl animate-in zoom-in-95 duration-300">
