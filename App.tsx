...
const App: React.FC = () => {
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'invite': return <Invite onNavigate={handleNavigate} profile={profile} />;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background-dark max-w-md mx-auto shadow-2xl overflow-x-hidden">
      ...
      {/* Bottom Navigation */}
      {session && ['home', 'shop', 'profile', 'invite', 'ganhos-tarefas'].includes(currentPage) && (
        <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-surface-dark/95 backdrop-blur-lg border-t border-amazon-border pb-1.5 pt-1.5 px-2 z-50">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className={`flex flex-col items-center gap-0.5 w-full transition-colors ${currentPage === 'invite' ? 'text-primary' : 'text-gray-500'}`}
            >
              <span className={`material-symbols-outlined text-[22px] ${currentPage === 'invite' ? 'fill-1' : ''}`} style={{ fontVariationSettings: `'FILL' ${currentPage === 'invite' ? 1 : 0}` }}>person_add</span>
              <span className="text-[9px] font-medium">Convite</span>
            </button>
