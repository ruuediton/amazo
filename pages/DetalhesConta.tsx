...
const DetalhesConta: React.FC<Props> = ({ onNavigate, showToast }) => {
  ...
  const fetchBankDetails = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('get_decrypted_bank_account');
            ...
            <>
                ...
                <div className="relative h-full flex flex-col p-5 z-10">
                  ...
                  <div className="mt-auto mb-2">
                    ...
                    <p className="text-[16px] font-mono font-bold text-black tracking-[0.08em] drop-shadow-md select-all leading-none">
                      {bankInfo?.numero_iban?.match(/.{1,4}/g)?.join(' ')}
                    </p>
                  </div>
              ...
              <div className="mt-10 flex flex-col gap-4 px-2">
                ...
                <p className="text-center text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] leading-relaxed mt-4 px-6 opacity-60">
                  ESTE CARTÃO É PROTEGIDO POR PROTOCOLOS DE SEGURANÇA BANCÁRIA DA amazon AO
                </p>
              </div>
            </>
          )}
