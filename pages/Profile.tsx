    ...
    <div className="flex flex-col pb-32 bg-white text-[#111418] font-display overflow-x-hidden">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <h1 className="text-2xl font-extrabold tracking-tight text-[#111418]">
              {profile.nome || 'Usuário amazon'}
            </h1>
            <div className="flex flex-col mt-1 space-y-0.5">
              <p className="text-[#637381] text-sm font-medium">ID: {profile.code}</p>
            ...
            <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1 bg-[#f4d125] rounded-full">
              ...
              <span className="text-[#111418] text-[10px] font-extrabold uppercase tracking-wider">
                {profile.verified ? 'Membro Prime' : 'Membro Prime'}
              </span>
            </div>
          </div>
        ...
        <div className="rounded-2xl bg-[#fefce8] p-6 border border-[#f4d125]/20 shadow-sm relative overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-y-6 gap-x-1 mt-8 pt-4 border-t border-[#f4d125]/10">
            <div className="text-center">
              ...
              <p className="text-[13px] font-bold text-[#111418] mt-1">Kz {(profile.deposit_total || 0).toLocaleString('pt-AO')}</p>
              <div className="mt-4 pt-3 border-t border-[#f4d125]/5">
                ...
                <p className="text-[13px] font-bold text-[#111418] mt-1">Kz {(profile.total_invested || 0).toLocaleString('pt-AO')}</p>
              </div>
            </div>
            <div className="text-center border-x border-[#f4d125]/10">
              ...
              <p className="text-[13px] font-bold text-[#111418] mt-1">Kz {(profile.withdraw_total || 0).toLocaleString('pt-AO')}</p>
              <div className="mt-4 pt-3 border-t border-[#f4d125]/5">
                <p className="text-[9px] uppercase text-[#637381] font-bold tracking-wider">Ganho de Equipe</p>
                <p className="text-[13px] font-bold text-[#111418] mt-1">Kz {(profile.team_earnings || 0).toLocaleString('pt-AO')}</p>
              </div>
            </div>
            <div className="text-center">
              <p className="text-[9px] uppercase text-[#637381] font-bold tracking-wider">Renda Diária</p>
              <p className="text-[13px] font-bold text-green-600 mt-1">+Kz {(profile.daily_income || 0).toLocaleString('pt-AO')}</p>
              <div className="mt-4 pt-3 border-t border-[#f4d125]/5">
                <p className="text-[9px] uppercase text-[#637381] font-bold tracking-wider">Renda Mensal</p>
                <p className="text-[13px] font-bold text-green-600 mt-1">+Kz {(profile.monthly_income || 0).toLocaleString('pt-AO')}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="px-4 space-y-6">
        <section>
          ...
          <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
            {[
              ...
              { label: 'Recarregar VIP', icon: 'stars', page: 'deposit-usdt' },
              ...
              { label: 'Missões PI', icon: 'assignment', page: 'gift-chest' },
            ].map((item, i, arr) => (
              <div
                key={item.label}
        ...
        <section>
          ...
          <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
            {[
              { label: 'Tutoriais', icon: 'menu_book', page: 'tutorials' },
        ...
        <div className="pb-12 pt-4">
          <button
            onClick={onLogout}
            className="w-full py-4 text-center text-red-600 font-bold bg-[#fee2e2] active:bg-red-200 rounded-xl transition-colors"
          >
            Sair da Conta
          </button>
