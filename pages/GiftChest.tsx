                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <p className="text-black/50 text-xs text-left">
                        {new Date(item.created_at).toLocaleTimeString()} • {item.code}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={`font-black block ${item.status === 'success' ? 'text-green-600' : 'text-red-400'}`}>
                      {item.status === 'success' ? `+ Kz ${item.amount || 0}` : 'Inválido'}
                    </span>
                    <span className="text-[10px] text-black/40 font-bold uppercase">{new Date(item.created_at).toLocaleDateString()}</span>
                  </div>
                </div>
              ))
            ) : (
