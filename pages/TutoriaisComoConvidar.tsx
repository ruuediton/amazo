...
const TutoriaisComoConvidar: React.FC<Props> = ({ onNavigate }) => {
  const steps = [
    {
      ...
      icon: "content_copy",
      content: (
        <div className="bg-surface-dark border border-gray-200 rounded-3xl p-5 text-left">
          ...
          <button className="w-full bg-primary hover:bg-green-400 active:scale-[0.98] text-black font-black py-4 px-4 rounded-2xl flex items-center justify-center gap-2 transition-all">
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden max-w-md mx-auto bg-background-dark antialiased pb-32">
      {/* Header */}
      <header className="flex items-center justify-between px-4 py-4 sticky top-0 z-20 bg-background-dark/95 border-b border-gray-200">
        <button
...
const SocialIcon: React.FC<{ color?: string, gradient?: string, label?: string, icon?: string, name: string, textColor?: string, border?: boolean }> = ({ color, gradient, label, icon, name, textColor = "white", border }) => (
  <div className="flex flex-col items-center gap-2 group cursor-pointer">
    <div
      className={`size-12 rounded-2xl flex items-center justify-center group-hover:scale-105 transition-transform ${border ? 'border border-white/10' : ''}`}
